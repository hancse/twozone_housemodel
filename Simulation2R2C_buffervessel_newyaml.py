...
"""
from house_buffervessel_newyaml import house_buffervessel  # exposed function "house" in house module
...
def main():
    ...
    Qsolar = np.zeros((num_sim, num_nodes))  # 8760 rows x 2 cols
    for s in house_param['chains'][0]['Solar_irradiation']:
        ...
        for n in range(num_nodes):
            Qsolar[:, n] += (df_irr.total_irr).values * partfactor[n]
        time_sim = df_irr.iloc[0:480, 0].values  # 480 rows 1D

    Qsolar_sim = Qsolar[0:480, :].T   # 2 rows x 480 cols

    sources = house_param['chains'][0]['Sources']
    for src in sources:
        if src['name'] == 'Internal_load':
            # Q_internal = np.zeros(num_sim)  # 8760 rows x 1 cols
            Q_internal = internal_heat_gain(src['Set_load'][0],
                                                  src['Set_load'][1],
                                                  src['Set_time'][0],
                                                  src['Set_time'][1])
    Qinternal_sim = Q_internal[0:480].flatten()  # 2 rows x 480 cols

    T_outdoor = df_nen.loc[:, 'temperatuur'].values / 10.0  # temperature
    T_outdoor_sim = T_outdoor[0:480].flatten()  # 480 rows 1D
    ...
    ax1.plot(data[4], data[0], label='Tair')
    ax1.plot(data[4], data[1], label='Twall')
    ax1.plot(time_sim, SP_sim, label='SP_Temperature')
    ax1.set_ylabel('T ($\degree C$)')
    ax1.set_ylim(15, 25)

    ax2.plot(data [4],data[2], label='Treturn')
    ax2.plot(data [4],data[3], label='Tbuffervessel')
    ax2.set_ylabel('T ($\degree C$)')
    ax2.set_ylim(10, 90)

    ax3.plot(time_sim,T_outdoor_sim,label='Toutdoor')
